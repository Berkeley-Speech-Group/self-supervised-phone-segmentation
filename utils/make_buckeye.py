...
parser.add_argument('--min_phonemes', type=int, required=True)
parser.add_argument('--max_phonemes', type=int, required=True)
...
has_nonzero_duration = lambda x: x.dur > 0
...
for wav in tqdm(wavs):
    try:
        ...
        delim_locations = np.array([i for i, phone in enumerate(phones) if is_delim(phone) and has_nonzero_duration(phone)])
        ...
        for start, end in zip(delim_locations[:-1], delim_locations[1:]):
            segment = deepcopy(phones[start:end+1])
